...
Recently my girlfriend is watching SouthPark for the first time and we are both fans of tier lists. After a fairly quick look for tier list sites, I didn't find any that I liked that much and this felt like a good opportunity for a project to learn React. 
...
I broke this down into multiple parts to look at each problem as it is solved.
...
const loadAndSave = async (url, filename, delay) => {
    ...
    await fsP.writeFile(path.join(__dirname, filename), JSON.stringify(data));
}

loadAndSave("https://spapi.dev/api/episodes", 'episodes.json', 200)
loadAndSave("https://spapi.dev/api/characters", 'characters.json', 200)
```


## Part 2: Scaffolding the frontend
I decided on React so I could branch out from Angular. I have predominantly been an Angular developer, and this felt like a good opportunity to see a framework that is structured differently.
...
* Is it opinionated? This isn't bad but can dictate how easily you can modify it
#### Features
...
Integrating the DnD Kit was fairly straightforward using their examples. I did need to put in my own custom component for the episodes and knowing I might add on other tier lists later(I did add characters). To plan for this now I had them passed in as a prop as long as they implemented a specific interface. This is a useful strategy to make a component more reusable. 
...
## Part 5: Storing Data
Now that we know what data we need to store for ranking episodes, we can build out a backend for this. I chose to go with an express application written with typescript that would save data to google datastore. My reasons were I wanted to be able to comfortably sit in the google app engine free tier. 
This is a fairly straight forward set up, we have 3 main routes;
* Get my tier list
* Update my tier lists
* List some tier lists.

For get my tier list, we will return a default list if we dont have one.

Update my tier list, we check if one exists and if it doesn't we will create a new one.


## Part 6: Make a notification Bar
When designing a site, it's necessary to make sure the user knows when something goes wrong. I am new to React and a good way to better understand how it works is by creating commonly used elements. In this scenario I wanted to make a notification system. 

### The notification system has two parts to it.
* A way to enqueue a new message
* A consumer that displays the messages

I started by making a [Context](https://reactjs.org/docs/context.html) based service which is built on top of the subscriber pattern. I wanted to use a Context because it makes it easily accessible from any component and a subscriber so it is easy to have the notification component know when there is another message.
...
Below is a simple implementation for a subscriber pattern, subject and the notification service.
...
Here is the notification service.
